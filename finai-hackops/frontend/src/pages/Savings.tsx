...
const Savings = () => {
  ...
  useEffect(() => {
    const fetchSavingsGoals = async () => {
      ...
      try {
        // Mock data for now until backend endpoint is implemented
        setGoals([
          {
            id: 1,
        ...
        // const data = await apiClient.get(`/financial-goals/${user.id}`);
        // setGoals(data);
      } catch (error) {
  ...
  const handleAddGoal = async () => {
    ...
    toast.info("Goal management will be fully enabled soon (currently local only)");
              ...
              <Card key={goal.id} className="gradient-card shadow-card border-0 overflow-hidden">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    ...
                    {goal.auto_debit && goal.monthly_contribution && (
                      <div className="flex items-center space-x-2">
